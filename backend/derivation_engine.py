...
def propagate_from_nodes(graph, seed_ids: set) -> list:
    ...
    def _add(a, b, rtype):
        ...
        if rtype == 'spouse':
            ...
            if (pa and pa.is_placeholder) or (pb and pb.is_placeholder):
                return False
            # 安全阈值：除非明确指定，否则推导引擎不产生同性配偶（减少逻辑爆炸）
            if pa and pb and pa.gender == pb.gender and pa.gender != 'unknown':
                return False
        derived.add(key)
    ...
    while queue and it < 100:
        ...
        for pid in batch:
            ...
            # Rule 2: co-parent (Disabled to prevent errors in multi-spouse families)
            # for sp in spouses:
            #     for ch in children:
